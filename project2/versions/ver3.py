...
select_scale = 2 #run-select
run_time = 1 #run-time
ubtrial = 20 #path_scanning
scanning_time = 10 #generation population


class MyProcess(multiprocessing.Process):
    def __init__(self, population, usedTime, pi, q):
    ...
    def run(self):
        ...
        while time.time() - time_begin_run < (TERMINATION_TIME - self.usedTime)*run_time:
            newGen = select(int(size / select_scale), self.population)
            mutation_gen += 1
            Pbf = 1/mutation_gen
            for solution in newGen:
                solution[1] += crossover(solution[0], 30-3*self.pi, Pbf, crossover_Pro)
                solution[0] = list(filter(None, solution[0]))
                solution[1] += mutation(solution[0], Pbf, flip_pro, sig_in_pro)
                solution[0] = list(filter(None, solution[0]))
...
if __name__ == '__main__':
    P = 8
    population = path_scanning(TERMINATION_TIME / scanning_time, ubtrial)
    q = multiprocessing.Manager().Queue()
    jobs = []
    for proc in range(P):
        sub_Process = MyProcess(copy.deepcopy(population), time.time()-start_time, proc, q)
        sub_Process.start()
