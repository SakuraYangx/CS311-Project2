...
# TERMINATION_TIME -= 5
# RANDOM_SEED = 1.0
# f = open('D:\python\CS311\Project\project2\CARP\CARP_samples\egl-e1-A.dat', encoding='utf-8')
lines = []
paraLines = []
dataLines = []
for line in f:
...
class MyProcess(multiprocessing.Process):
    ...
    def run(self):
        ...
        while time.time() - time_begin_run < (TERMINATION_TIME - self.usedTime)*4/5:
            newGen = select(int(size / 2), self.population)
...
if __name__ == '__main__':
    ...
    populations = []
    for i in range(P):
        population = path_scanning(TERMINATION_TIME / 10, 50)
        populations.append(population)
    q = multiprocessing.Manager().Queue()
    jobs = []
    for proc in range(P):
        sub_Process = MyProcess(populations[proc], time.time()-start_time, proc, q)
        sub_Process.start()
